...
describe('Header & Navigation', () => {
  ...
  it(`\`main.css\` should contain the classes, \`${declarations.headerNav.selector}\`, \`${declarations.brand.selector}\`, and \`${declarations.logo.selector}\`, they should be used in \`index.html\` @header-brand-logo-class-attributes`, () => {
    assert(headerNav.class, 'The `.header-nav` class should exist before applying it in the HTML.');
    assert(brand.class, 'The `.brand` class should exist before it is used in the HTML.');
    assert(logo.class, 'The `.logo` class should exist before it is used in the HTML.');
  ...
  it(`\`main.css\` should contain the classes, \`${declarations.searchBox.selector}\`, and \`${declarations.searchButton.selector}\`, they should be used in \`index.html\` @search-class-attributes`, () => {
    assert(searchBox.class, 'The `.search-box` class should exist before applying it in the HTML.');
    assert(searchButton.class, 'The `.search-button` class should exist before it is used in the HTML.');
  ...
  it(`\`main.css\` should contain a \`${declarations.navList.selector}\` class, it should be used in \`index.html\` @navigation-list-class-attribute`, () => {
    assert(navList.class, 'The `.nav-list` class should exist before applying it in the HTML.');
  ...
  it(`\`main.css\` should contain the classes, \`${declarations.profile.selector}\`, \`${declarations.account.selector}\`, and \`${declarations.profilePic.selector}\`, they should be used in \`index.html\` @profile-class-attribute`, () => {
    assert(profile.class, 'The `.profile` class should exist before applying it in the HTML.');
    assert(account.class, 'The `.account` class should exist before it is used in the HTML.');
    assert(profilePic.class, 'The `.profile-pic` class should exist before applying it in the HTML.');
